...
def result():
    ...
    key_encrypt = request.form['key_encrypt']
    key_decrypt = request.form["key_decrypt"]
    encrypted = ''
    decrypted = ''
    if mode == 'encrypt':
        gost = Crypt(int(key_encrypt), blocks)
        cyphred = []
        for x in text:
            cyphred.append(gost.encrypt(ord(x)))
        encrypted = " ".join([str(i) for i in cyphred])
    if mode == "decrypt":
        decyphred = []
        gost = Crypt(int(key_decrypt), blocks)
        for x in text.split():
                decyphred.append(gost.decrypt(int(x)))
        decrypted = ''.join([chr(i) for i in decyphred])
    return render_template('index.html', text=text, key_encrypt=key_encrypt, key_decrypt=key_decrypt, encrypted=encrypted, decrypted=decrypted)
